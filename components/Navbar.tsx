                    ...
                    <div className="mx-auto flex w-full max-w-6xl flex-col gap-1">
                        {navigationLinks.map((link) => (
                            <Link
                                ...
                                href={link.href}
                                aria-current={isActive(link.href) ? 'page' : undefined}
                                className={`rounded-md px-3 py-3 text-sm transition-colors hover:bg-white/[.06] hover:text-zinc-50 ${isActive(link.href) ? 'bg-white/[.06] text-zinc-50' : 'text-zinc-400'
                                    }`}
                                onClick={() => setIsMenuOpen(false)}
                            >
                                {isActive(link.href) && <span className="mr-2 text-emerald-400">&gt;</span>}
                                {link.label}
                            </Link>
                        ))}
                        <Link
